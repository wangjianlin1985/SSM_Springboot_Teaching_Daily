...
public class TeacherController extends BaseController {
	...
	public void list(String teacherNo,@ModelAttribute("classObj") ClassInfo classObj,String name,String birthDate,String telephone,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<Teacher> teacherList = teacherService.queryTeacher(teacherNo, classObj, name, birthDate, telephone, page);
	    /*计算总的页数和总的记录数*/
	    teacherService.queryTotalPageAndRecordNumber(teacherNo, classObj, name, birthDate, telephone);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String teacherNo,@ModelAttribute("classObj") ClassInfo classObj,String name,String birthDate,String telephone,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (teacherNo == null) teacherNo = "";
		if (name == null) name = "";
		if (birthDate == null) birthDate = "";
		if (telephone == null) telephone = "";
		List<Teacher> teacherList = teacherService.queryTeacher(teacherNo, classObj, name, birthDate, telephone, currentPage);
	    /*计算总的页数和总的记录数*/
	    teacherService.queryTotalPageAndRecordNumber(teacherNo, classObj, name, birthDate, telephone);
	    /*获取到总的页码数目*/
	    int totalPage = teacherService.getTotalPage();
	...
	public void OutToExcel(String teacherNo,@ModelAttribute("classObj") ClassInfo classObj,String name,String birthDate,String telephone, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(teacherNo == null) teacherNo = "";
        if(name == null) name = "";
        if(birthDate == null) birthDate = "";
        if(telephone == null) telephone = "";
        List<Teacher> teacherList = teacherService.queryTeacher(teacherNo,classObj,name,birthDate,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Teacher信息记录"; 
        String[] headers = { "教师工号","所在班级","姓名","性别","出生日期","教师照片","联系电话"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<teacherList.size();i++) {
        	Teacher teacher = teacherList.get(i); 
        	dataset.add(new String[]{teacher.getTeacherNo(),teacher.getClassObj().getClassName(),teacher.getName(),teacher.getGender(),teacher.getBirthDate(),teacher.getTeacherPhoto(),teacher.getTelephone()});
        }
        /*
