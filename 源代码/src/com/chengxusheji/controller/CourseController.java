...
public class CourseController extends BaseController {
	...
	public void teacherAdd(@Validated Course course, BindingResult br,
		...
		course.setTeacherObj(teacher);
		
		course.setClassObj(teacher.getClassObj());
	...
	public void list(String courseNo,String courseName,@ModelAttribute("teacherObj") Teacher teacherObj,@ModelAttribute("classObj") ClassInfo classObj,String addTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (courseNo == null) courseNo = "";
		if (courseName == null) courseName = "";
		if (addTime == null) addTime = "";
		if(rows != 0)courseService.setRows(rows);
		List<Course> courseList = courseService.queryCourse(courseNo, courseName, teacherObj, classObj, addTime, page);
	    /*计算总的页数和总的记录数*/
	    courseService.queryTotalPageAndRecordNumber(courseNo, courseName, teacherObj, classObj, addTime);
	    /*获取到总的页码数目*/
	...
	public void teacherlist(String courseNo,String courseName,@ModelAttribute("teacherObj") Teacher teacherObj,@ModelAttribute("classObj") ClassInfo classObj,String addTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response,HttpSession session) throws Exception {
		if (page==null || page == 0) page = 1;
		if (courseNo == null) courseNo = "";
		if (courseName == null) courseName = "";
		if (addTime == null) addTime = "";
	...
	/*ajax方式按照查询条件分页查询课程信息*/
	@RequestMapping(value = { "/teacherListAll" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void teacherListAll(HttpServletResponse response,HttpSession session) throws Exception {
		String teacherNo = session.getAttribute("username").toString();
		List<Course> courseList = courseService.queryTeacherAllCourse(teacherNo);
        response.setContentType("text/json;charset=UTF-8"); 
		PrintWriter out = response.getWriter();
		JSONArray jsonArray = new JSONArray();
		for(Course course:courseList) {
	...
	public String frontlist(String courseNo,String courseName,@ModelAttribute("teacherObj") Teacher teacherObj,@ModelAttribute("classObj") ClassInfo classObj,String addTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (courseNo == null) courseNo = "";
		if (courseName == null) courseName = "";
		if (addTime == null) addTime = "";
		List<Course> courseList = courseService.queryCourse(courseNo, courseName, teacherObj, classObj, addTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    courseService.queryTotalPageAndRecordNumber(courseNo, courseName, teacherObj, classObj, addTime);
	    /*获取到总的页码数目*/
	    int totalPage = courseService.getTotalPage();
	...
	public void OutToExcel(String courseNo,String courseName,@ModelAttribute("teacherObj") Teacher teacherObj,@ModelAttribute("classObj") ClassInfo classObj,String addTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(courseNo == null) courseNo = "";
        if(courseName == null) courseName = "";
        if(addTime == null) addTime = "";
        List<Course> courseList = courseService.queryCourse(courseNo,courseName,teacherObj,classObj,addTime);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Course信息记录"; 
        String[] headers = { "课程编号","课程名称","课程照片","总课时","课程学分","上课老师","上课班级","发布时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<courseList.size();i++) {
        	Course course = courseList.get(i); 
        	dataset.add(new String[]{course.getCourseNo(),course.getCourseName(),course.getCoursePhoto(),course.getCourseHours() + "",course.getCourseScore() + "",course.getTeacherObj().getName(),course.getClassObj().getClassName(),course.getAddTime()});
        }
        /*
